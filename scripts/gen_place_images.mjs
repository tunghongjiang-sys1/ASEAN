// Generates src/data/place-images.ts from assets/<Country>/<country>_N.* photos.
// Photos are numbered by their position in the data file (indonesia_1 = 1st place,
// indonesia_2 = 2nd place, ...). Run: node scripts/gen_place_images.mjs
//
// Mapping modes:
//   number  — photo number in the filename = place number (indonesia_14 -> 14th place).
//   order   — photos are in folder order; the Nth file (by mtime) maps to the Nth place,
//             filename numbers are ignored. Used for Vietnam per user request.
...
for (const { folder, list, prefix, mode } of COUNTRIES) {
  ...
  if (mode === 'order') {
    // Vietnam: photos are in folder order — ignore filename numbers, use mtime order.
    files.sort((a, b) => mtimeMs(a, dir) - mtimeMs(b, dir));
  } else {
    files.sort((a, b) => numericName(a) - numericName(b));
  }

  // photo number = place number (indonesia_14 -> the 14th place, index 13)
...
const output = `// AUTO-GENERATED by scripts/gen_place_images.mjs — do not edit by hand.
